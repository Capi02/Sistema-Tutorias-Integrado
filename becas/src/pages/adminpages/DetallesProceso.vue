    ...
    <div class="q-pa-md row items-start q-gutter-md">
      <div class="q-gutter-md container">
        <q-form @submit="onSubmit" @reset="onReset">
          <div class="q-pt-lg">
            <div class="q-col-gutter-md q-row items-start">
              <div class="row">
              <q-col cols="4">
                <q-card class="my-card">
                <q-card-section class="q-col-md-4">
                  <div class="q-pa-md">
                  <p class="q-mb-sm">Los datos de la solicitud son:</p>
                  <!-- Mostrar los detalles del registro -->
                  <p class="q-mb-sm"><strong>Apellido Paterno:</strong> {{ formData && formData.apellidopat }}</p>
                  <p class="q-mb-sm"><strong>Apellido Materno:</strong> {{ formData && formData.apellidomat }}</p>
            ...
            <q-col cols="4">
              ...
              <q-card-section class="q-col-md-4">
                <div class="q-pa-md">
                  <p class="q-mb-sm">
                    Los datos enviados de los socioeconomicos del estudiante son:
                  </p>
                  <div class="q-mb-sm">
            ...
            <q-col cols="4">
                ...
                <q-card-section class="q-col-md-4">
                  <div class="q-pa-md">
                    <p class="q-mb-sm">Los datos del solicitante son:</p>
                    <div class="q-mb-sm">
                      <strong>Nombre:</strong>
            ...
            <q-card class="my-card">
              ...
              <q-card-section>
                <div class="q-pa-md">
                  <p class="q-mb-sm"><strong>Documentos:</strong></p>
                  <div v-for="documento in formDataDocumentos" :key="documento.iddocumento" class="q-mb-sm">
                    ...
                    <a :href="`http://127.0.0.1:3000/api/documentos/${formData.studentId}/${documento.nombre}`" target="_blank">Ver documento</a>
                  </div>
                </div>
              </q-card-section>
            </q-card>
              <q-card-section>
                <p class="q-mb-sm"><strong>Estado de la beca:</strong> {{ getEstadoContent(formData && formData.idestado) }}</p>
              ...
              <q-card-section>
                ...
                <div class="q-gutter-md">
                  <q-btn
                    ...
                    :key="estado.value"
                    v-model="estadoRef"
                    @click="estadoRef = estado"
                    :color="estadoRef === estado ? 'primary' : 'default'"
                    :label="estado.label"
                    class="q-mb-md"
                    text-color="black"
                    color:primary
                  ...
                  <p>¡En caso de ya sea aprovada la beca y deben asignarle un procentaje a la becas y si no se aprueba asignarle el valor de cero!</p>
                <q-btn type="submit" color="primary" label="Guardar" class="q-mt-md" @click="onSubmit" />
              </q-card-section>
            </div>
            </div>
          </div>
...
export default{
  ...
  setup (props) {
    ...
    const getCarreraContent = (idcarrera) => {
        ...
        const studentId = ref(); // Create a ref to hold the studentId value
  ...
  try {
    ...
    const response = await axios.get(`http://127.0.0.1:3000/api/solicitud/${props.idsolicitud}`);
    formData.value = response.data;
    estadoRef.value = formData.value.idestado;
    formData.value.studentId = studentId.value;

    const responseDocumentos = await axios.get(`http://127.0.0.1:3000/api/documentos/${studentId.value}`);
    formDataDocumentos.value = responseDocumentos.data;

    const cartaResponse = await axios.get(`http://127.0.0.1:3000/api/carta/estatic/${studentId.value}`);
    formDataCarta.value = cartaResponse.data[0];

    const socioResponse = await axios.get(`http://127.0.0.1:3000/api/socioeconomico/estatic/${studentId.value}`);
    formDataSocio.value = socioResponse.data[0];
  } catch (error) {
