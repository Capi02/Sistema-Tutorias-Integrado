<template>
  <q-page class="q-pa-md container" padding>
    <p class="title">Beca Inclusión</p>
    ...
    <div class="q-pa-xs q-pt-lg">
      <q-card class="my-card q-card-bordered">
        ...
        <q-space />
        <q-file filled bottom-slots class="col-5 q-pt-lg" v-model="medico" name="pdfFiles" label="Dictamen médico, certificado de la preparatoria o historial académico" counter max-files="12" accept="application/pdf">
          <q-btn round dense flat icon="add" @click.stop.prevent />
        </q-file>
        <q-space />
        <q-toggle
  ...
  setup() {
    ...
    async function onSubmit() {
      ...
      try {
        ...
        router.push("/alumno/inclusion/thk"),
        );
